...
* исправить функции в соответствии с новым форматом xml-файла
нужно протестировать на виртуальной машине как lsblk отображается различные ситуации
* добавим ключ для использования альтернативного xml-файла для тестирования на виртуальной машине
* перенести логику вывода информации пользователю (нужно подумать оставить ли 
её в том виде, ли тот код нужно переделать)
* т.к. сначала лучше открыть все крипто-контейнеры (которые не планируются к созданию),
записываем вывод lsblk после обхода всех точек монтирования
* можем скопировать уже реализованный в прошлом фунционал, только надо учесть:
* нужно заполнить поле device т.к. раньше использовалось она
* нужно дописывать туда информацию о крипто-контейнерах, которые планируются к созданию
(но по сути это будет в случаях аналогичных созданию и форматированию ext4, просто всё зависит
опции crypt_mode)
