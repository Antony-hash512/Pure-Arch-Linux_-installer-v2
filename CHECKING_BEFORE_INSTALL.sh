...
: <<'TODO'
* в рамках борьбы с говнокодом, нужно переделать
функции get_new_btrfs_subvolumes_for_device_with_their_mount_points
и get_new_lvm_logical_volumes_for_device_with_their_mount_points
(например нужно убрать чистку мусора за результатом функций, вместо этого нужно создать два вида функций:
1) для вывода заполненения ассоциативного массива, полученного по ссылке
(данные из этого массива будут использоваться для сравнения в условии и т.д.)
2) для вывода информации пользователю c точкой монтирования в заданном формате
(с использованием данных уже заполненного ассоциативного массива)
* выделить трудночитаемые операции со строками в отдельные функции с понятными названиями
