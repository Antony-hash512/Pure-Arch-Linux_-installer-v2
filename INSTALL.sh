...
: <<'TODO'
* разобраться как настроить /etc/crypttab для none_in_*: провести траблшутинг (описан в заметке Obsidian)
    * исправить в соответствии с https://wiki.archlinux.org/title/Dm-crypt/Encrypting_an_entire_system
        * выяснилось что для none_in_* не нужно ничего писать в /etc/crypttab, вот и возникала ошибка :))
        * нужно переписать функцию configure_crypt_volumes_by_ref в соответствии с моими новыми заметками в Obsidian
    * протестировать исправленный вариант на vm
...
* дописать буты и расширения архивов homefiles
