...
: <<'TODO'
* обновить кастомные конфиги для hyprland, не забыть добавить туда конфиг mvp
* добавить тест с шифрованным btrfs на разделе
* отделить конфиг, отвечающий за разметку диска, от components.xml: 2 новых файла 1) пользовательский в gitignore, 2) с тестами в репе
* сделать скрипт для предъустновочной разметки диска и установок точек монтирования
* сделать пользователься components.xml в gitignore, по-умолчанию он копируется из оригинального components_example.xml в моими наборами пакетов
* написать скрипт для разметки диска с legacy bios
* реализовать поддержку старых ноутбуков с legacy bios в самом скрипте
* проверить открытия luksов по кейфайлу (в случаях с ext4 может быть создан новый кейфайл, в случаях с btrfs нет)
* проверить: если /home прописана в качестве уже существующего каталога и такой пользователь существует, то копировать конфиги туда не надо
* добавить в конфиг опциональную настройку гита user.name и user.email, их можно дописывать в ~/.gitconfig после распаковки архивов

* зарелизить бету !!!! <-- ВАЖНО НАКОНЕЦ-ТО НАДО ЗАРЕЛИЗИТЬ ГОТОВЫЙ MVP
* заменить openbox на labwc в конфиге
* составить набор пакетов для рабочего стола cosmic
* оптимизировать способ хранения списков пакетов (новые можно создавть на базе предыдующих, добавляя или убирая определённые пакеты)
* установка grub с определёнными модулями
* удалять кеш рефлектора если ему больше суток (перестало быть актуальным, если используется кеш хоста)
