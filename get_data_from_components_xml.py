...
DEFAULT_XML_FILE = 'components.xml'  # Значение по умолчанию для XML файла
...
# Создаем подпарсеры для разных форм вызова
subparsers = parser.add_subparsers(dest='command_type', help='Режим работы')

# Подпарсер для списка компонентов
list_parser = subparsers.add_parser('list', help='Получить список компонентов')
list_parser.add_argument('component_type', choices=['driverspack', 'softpack', 'install_location', 'settings'], 
                        help='Тип компонента для вывода списка')

# Подпарсер для обработки компонента
process_parser = subparsers.add_parser('process', help='Обработать конкретный компонент')
process_parser.add_argument('component_type', help='Тип компонента (driverspack, softpack, install_location, settings)')
process_parser.add_argument('component_id', help='ID компонента')
process_parser.add_argument('command', help='Команда для выполнения')
process_parser.add_argument('optional_args', nargs='*', help='Дополнительные аргументы для команды')
...
def main():
    # Передаем оставшиеся аргументы в sys.argv
    sys.argv = [sys.argv[0]] + args.optional_args
