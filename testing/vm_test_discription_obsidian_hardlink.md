...
## новые тесты в которых создаётся новый кейфайл

#   Траблшутинг
```

Проблема вовсе не в том, что вы неправильно открываете LUKS-контейнер — initramfs вполне успешно расшифровывает PV и получает устройство `/dev/mapper/crypt_<UUID>`. Дальше он по параметру `root=` пытается сразу смонтировать это устройство как «корневую» файловую систему. Но на самом деле внутри этого LUKS-контейнера лежит LVM-PV, а не сама файловая система — поэтому `mount` жалуется, что видит там тип «LVM2_member» и не знает, как с ним работать.

Иными словами, в вашем `GRUB_CMDLINE_LINUX` (который формируется функцией `configure_crypt_volumes_by_uuid`) вы прописываете

    cryptdevice=UUID=…:crypt_<UUID> root=/dev/mapper/crypt_<UUID>

а нужно указывать в `root=` уже ваш LVM-том или (для Btrfs-сабволюма) логический том вместе с опцией `rootflags=subvol=@arch1`. Например, для `vm_test1` должно быть что-то вроде:
